import { STEPS, FINAL, TARGETS, CHROME, IMG } from './coworking-en-data.js';
import { setupQuestNavigation } from './quest-navigation.js';

const $ = id => document.getElementById(id);
const AUDIO_DIR = '../assets/coworking-en/audio/';
...
function showStep(push = true) {
  ...
  const q = $('question');
  const kind = l => l.who === 'narr' ? 'narr' : l.who === 'Lucy' ? 'lucia' : l.who === 'You' ? 'hero' : 'other';
  ...
  if (step.type === 'choice') {
    ...
    order.forEach((c, i) => {
      ...
      if (isImg) {
        b.setAttribute('aria-label', 'Option ' + 'ABC'[i]);
...
function showFinal(push = true) {
  ...
  $('journal').innerHTML = TARGETS.map(t => {
    ...
    return `<section class="jw"><h3>${t.en}</h3>${us.map(u => `<div class="ju"><p class="jc">${u.text ?? 'Option ' + u.letter}</p><p class="jr">${u.react}</p></div>`).join('')}</section>`;
  }).join('');
...
$('vocab-list').innerHTML = TARGETS.map(t => `<li><b>${t.en}</b> — ${t.ru}</li>`).join('');
